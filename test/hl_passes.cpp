...
TEST(HlPassesTest, divisionImplementation) {
    ...
    std::string ep = "main";
    hl_pass_manager manager = create_hl_pass_manager(ep, {1,2,3,8,9,10,4,5,11});
...
TEST(HlPassesTest, loop_unrolling_array) {
    ...
    std::string ep = "main";
    hl_pass_manager manager = create_hl_pass_manager(ep,{1,2,3,8,4,5,6,7});
    manager.run_morphing_passes(parser.AST);
...
TEST(HlPassesTest, test_matrix_scalarization) {
    ...
    std::string ep = "main";
    hl_pass_manager manager = create_hl_pass_manager(ep,{1,2,3,8,9,10,4,5,6,7});
    manager.run_morphing_passes(parser.AST);
