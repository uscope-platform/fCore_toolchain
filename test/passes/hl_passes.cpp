...
TEST(HlPassesTest, divisionImplementation) {
    ...
    std::shared_ptr<ast_code_block> input_root = std::make_shared<ast_code_block>();

    std::shared_ptr<variable> var = std::make_shared<variable>("a");

    std::shared_ptr<ast_definition> define = std::make_shared<ast_definition>("a", c_type_float, var);

    std::shared_ptr<ast_expression> exp = std::make_shared<ast_expression>(ast_expression::DIV);
    ...
    std::shared_ptr<ast_code_block> result =  manager.run_optimizations(input_root, {});

    std::shared_ptr<ast_expression> mult_exp = std::make_shared<ast_expression>(ast_expression::MULT);


    var = std::make_shared<variable>("constant", 4);
    std::shared_ptr<ast_operand> op_1= std::make_shared<ast_operand>(var);
    mult_exp->set_lhs(op_1);

    var = std::make_shared<variable>("constant",6);
    op_1= std::make_shared<ast_operand>(var);
    std::shared_ptr<ast_expression> rec_exp = std::make_shared<ast_expression>(ast_expression::RECIPROCAL);
    rec_exp->set_rhs(op_1);

    mult_exp->set_rhs(rec_exp);

    var = std::make_shared<variable>("a");
    std::shared_ptr<ast_definition> mult_def = std::make_shared<ast_definition>("a", c_type_float, var);
    mult_def->set_scalar_initializer(mult_exp);


    std::shared_ptr<ast_code_block> gold_standard= std::make_shared<ast_code_block>();
...
TEST(HlPassesTest, intrinsics_implementation) {
    ...
    std::shared_ptr<ast_function_def> main_fcn = std::make_shared<ast_function_def>();
    main_fcn->set_name("main");
    main_fcn->set_return_type(c_type_int);

    std::shared_ptr<variable> var = std::make_shared<variable>("b");
    std::shared_ptr<ast_definition> def_1 = std::make_shared<ast_definition>("b", c_type_float, var);
    var = std::make_shared<variable>("c");
    std::shared_ptr<ast_definition> def_2 = std::make_shared<ast_definition>("c", c_type_float, var);

    var = std::make_shared<variable>("a");
    var->set_variable_class({variable_output_type, false});
    std::shared_ptr<ast_operand> op = std::make_shared<ast_operand>(var);

    std::vector<std::shared_ptr<ast_node>> args = {op};
    std::shared_ptr<ast_call> call_1 = std::make_shared<ast_call>("itf", args);
    ...
    op = std::make_shared<ast_operand>(var);
    args.push_back(op);
    std::shared_ptr<ast_call> call_2 = std::make_shared<ast_call>("satp", args);
    ...
    std::shared_ptr<ast_definition> def_3 = std::make_shared<ast_definition>("b", c_type_float, var);

    var = std::make_shared<variable>("a");
    var->set_variable_class({variable_output_type, false});
    std::shared_ptr<ast_operand> op_1 = std::make_shared<ast_operand>(var);

    std::shared_ptr<ast_expression> ex_1= std::make_shared<ast_expression>(ast_expression::ITF);
    ex_1->set_rhs(op_1);
    def_3->set_scalar_initializer(ex_1);

    var = std::make_shared<variable>("c");
    std::shared_ptr<ast_definition> def_4 = std::make_shared<ast_definition>("c", c_type_float, var);
    ...
    var = std::make_shared<variable>("constant",100);
    std::shared_ptr<ast_operand> op_2 = std::make_shared<ast_operand>(var);

    ex_1= std::make_shared<ast_expression>(ast_expression::SATP);
    ex_1->set_lhs(op_1);
    ex_1->set_rhs(op_2);
...
TEST(HlPassesTest, test_operating_assignments_implementation) {
    ...
    std::shared_ptr<ast_expression> exp = std::make_shared<ast_expression>(ast_expression::ASSIGN);
    exp->set_assignment_type(ast_expression::addition_assignment);
    exp->set_lhs(op);

    std::shared_ptr<ast_expression> expmul = std::make_shared<ast_expression>(ast_expression::MULT);
    expmul->set_lhs(op);
    ...
    const std::shared_ptr<ast_code_block>& result = manager.run_optimizations(input_root, {});

    std::shared_ptr<ast_code_block> gold_standard= std::make_shared<ast_code_block>();

    var = std::make_shared<variable>("c");
    std::shared_ptr<ast_expression> exadd = std::make_shared<ast_expression>(ast_expression::ADD);

    std::shared_ptr<ast_expression> exmul = std::make_shared<ast_expression>(ast_expression::MULT);
    ...
    std::shared_ptr<ast_expression> exassign = std::make_shared<ast_expression>(ast_expression::ASSIGN);
...
TEST(HlPassesTest, function_inlining) {
    ...
    std::shared_ptr<ast_function_def> function = std::make_shared<ast_function_def>();
    function->set_name("add_1");
    function->set_return_type(c_type_int);

    std::vector<std::shared_ptr<ast_definition>> parameters;

    std::shared_ptr<variable> var = std::make_shared<variable>("a");
    std::shared_ptr<ast_definition> def = std::make_shared<ast_definition>("a", c_type_int, var);
    parameters.push_back(def);
    std::shared_ptr<ast_operand> op_1 = std::make_shared<ast_operand>(var);

    var = std::make_shared<variable>("b");
    def = std::make_shared<ast_definition>("b", c_type_int, var);
    parameters.push_back(def);
    std::shared_ptr<ast_operand> op_2 = std::make_shared<ast_operand>(var);

    std::shared_ptr<ast_expression> exp_1 = std::make_shared<ast_expression>(ast_expression::ADD);
    exp_1->set_lhs(op_1);
    exp_1->set_rhs(op_2);

    var = std::make_shared<variable>("c");
    def = std::make_shared<ast_definition>("c", c_type_int, var);
    def->set_scalar_initializer(exp_1);

    op_1 = std::make_shared<ast_operand>(var);
    exp_1 = std::make_shared<ast_expression>(ast_expression::ADD);
    exp_1->set_lhs(op_1);
    exp_1->set_rhs(op_2);
    ...
    function= std::make_shared<ast_function_def>();
    ...
    def = std::make_shared<ast_definition>("e", c_type_int, var);
    parameters.push_back(def);
    std::shared_ptr<ast_operand> op_e = std::make_shared<ast_operand>(var);

    var = std::make_shared<variable>("f");
    def = std::make_shared<ast_definition>("f", c_type_int, var);
    parameters.push_back(def);
    std::shared_ptr<ast_operand> op_f = std::make_shared<ast_operand>(var);

    exp_1 = std::make_shared<ast_expression>(ast_expression::ADD);
    exp_1->set_lhs(op_e);
    exp_1->set_rhs(op_f);

    std::shared_ptr<ast_expression> exp_2 = std::make_shared<ast_expression>(ast_expression::ADD);
    exp_2->set_lhs(exp_1);
    exp_2->set_rhs(op_f);
    ...
    std::vector<std::shared_ptr<ast_node>> args = {op_1, op_2};
    std::shared_ptr<ast_call> call_1 = std::make_shared<ast_call>("add_1", args);
    std::shared_ptr<ast_call> call_2 = std::make_shared<ast_call>("add_2", args);
    ...
    const std::shared_ptr<ast_code_block>& res =  manager.run_optimizations(input_root, {});

    std::shared_ptr<ast_code_block> gold_standard= std::make_shared<ast_code_block>();

    // CALL BODY
    var = std::make_shared<variable>("_fcmglr_function_add_1_c");
    def = std::make_shared<ast_definition>("_fcmglr_function_add_1_c", c_type_int, var);
    std::shared_ptr<ast_expression> exadd = std::make_shared<ast_expression>(ast_expression::ADD);
    ...
    std::shared_ptr<ast_expression> ex_add2 = std::make_shared<ast_expression>(ast_expression::ADD);
    ex_add2->set_lhs(exadd);
    ex_add2->set_rhs(op_2);

    exadd = std::make_shared<ast_expression>(ast_expression::ADD);
...
TEST(HlPassesTest, simple_normalization) {
    ...
    std::shared_ptr<ast_node> raw_result = p.process_global(opt_result, globals);

    std::shared_ptr<variable> var = std::make_shared<variable>("intermediate_expression_0");
    std::shared_ptr<ast_definition> def_1 = std::make_shared<ast_definition>("intermediate_expression_0", c_type_float, var);
    ...
    std::shared_ptr<ast_expression> ex_1= std::make_shared<ast_expression>(ast_expression::MULT);
    ex_1->set_lhs(op_1);
    ex_1->set_rhs(op_2);
    ...
    var = std::make_shared<variable>("a");
    var->set_variable_class({variable_output_type, false});
    std::shared_ptr<ast_definition> def_2 = std::make_shared<ast_definition>("a", c_type_int, var);
    ...
    ex_1= std::make_shared<ast_expression>(ast_expression::ADD);
    ex_1->set_lhs(op_1);
    ex_1->set_rhs(op_2);
...
TEST(HlPassesTest, hl_ast_lowering) {
    ...
    std::shared_ptr<ast_code_block> normalized_ast = manager.run_optimizations(parser.AST, parser.get_globals());
...
TEST(HlPassesTest, loop_unrolling_array) {
    ...
    for(int i = 5; i<7; ++i){
        //assignment
        std::shared_ptr<ast_expression> assign_exp = std::make_shared<ast_expression>(ast_expression::ASSIGN);

        std::shared_ptr<variable> def_var = std::make_shared<variable>("j");
        def_var->set_type(var_type_array);
        std::shared_ptr<ast_operand> assign_op = std::make_shared<ast_operand>(def_var);
        std::shared_ptr<variable> idx_var = std::make_shared<variable>("i", i);
        std::shared_ptr<ast_operand> op_idx = std::make_shared<ast_operand>(idx_var);
        ...
        def_var->set_type(var_type_array);
        std::shared_ptr<ast_operand> op = std::make_shared<ast_operand>(def_var);
        idx_var = std::make_shared<variable>("i", i);
        op_idx = std::make_shared<ast_operand>(idx_var);
        op->set_array_index({op_idx});
        expr->set_lhs(op);
        //RHS
        def_var = std::make_shared<variable>("h");
        def_var->set_type(var_type_array);
        op = std::make_shared<ast_operand>(def_var);
        idx_var = std::make_shared<variable>("i", i);
        op_idx = std::make_shared<ast_operand>(idx_var);
        op->set_array_index({op_idx});
        expr->set_rhs(op);
...
TEST(HlPassesTest, test_matrix_scalarization) {
    ...
    std::shared_ptr<ast_node> normalized_ast = manager.run_optimizations(parser.AST, parser.get_globals());
    ...
    std::shared_ptr<ast_expression> ex = std::make_shared<ast_expression>(ast_expression::ASSIGN);
    ...
    std::shared_ptr<ast_expression> ex_inner = std::make_shared<ast_expression>(ast_expression::MULT);
    var = std::make_shared<variable>("b");
    var->set_type(var_type_array);
    ...
    ex = std::make_shared<ast_expression>(ast_expression::ASSIGN);
    ...
    ex_inner = std::make_shared<ast_expression>(ast_expression::MULT);
    var = std::make_shared<variable>("a");
    var->set_type(var_type_array);
    var->set_array_shape({10,2});
...
TEST(HlPassesTest, function_inlining_array) {
    ...
    std::shared_ptr<ast_node> normalized_ast = result_ast->get_content()[5];

    std::shared_ptr<ast_expression> gold_standard = std::make_shared<ast_expression>(ast_expression::ASSIGN);

    std::shared_ptr<variable> var = std::make_shared<variable>("_fcmglr_flattened_array_c_1_1");
    std::shared_ptr<ast_operand> op = std::make_shared<ast_operand>(var);
    gold_standard->set_lhs(op);

    std::shared_ptr<ast_expression> exp = std::make_shared<ast_expression>(ast_expression::ADD);
    var = std::make_shared<variable>("constant", 5.0f);
    op = std::make_shared<ast_operand>(var);
    exp->set_lhs(op);
...
TEST(HlPassesTest, function_return_inlining) {
    ...
    std::shared_ptr<ast_definition> def = std::make_shared<ast_definition>("inlined_variable_0", c_type_float, var);
    ...
    var->set_variable_class({variable_output_type, false});
    def = std::make_shared<ast_definition>("ret", c_type_float, var);
    std::shared_ptr<ast_expression> exp = std::make_shared<ast_expression>(ast_expression::ADD);
...
TEST(HlPassesTest, complex_normalization) {
    ...
    std::shared_ptr<ast_expression> expr_1 = std::make_shared<ast_expression>(ast_expression::MULT);

    std::shared_ptr<variable> var = std::make_shared<variable>("fwd_in");
    std::shared_ptr<ast_operand> fwd_op = std::make_shared<ast_operand>(var);
    expr_1->set_rhs(fwd_op);

    var = std::make_shared<variable>("Ts");
    std::shared_ptr<ast_operand> ts_op = std::make_shared<ast_operand>(var);
    expr_1->set_lhs(ts_op);
    // OUTER MULTIPLICATION
    std::shared_ptr<ast_expression> expr_2 = std::make_shared<ast_expression>(ast_expression::MULT);
    expr_2->set_lhs(expr_1);

    var = std::make_shared<variable>("omega");
    std::shared_ptr<ast_operand> w_op = std::make_shared<ast_operand>(var);
    expr_2->set_rhs(w_op);
    // ADDITION
    expr_1 = std::make_shared<ast_expression>(ast_expression::ADD);
    expr_1->set_rhs(expr_2);

    var = std::make_shared<variable>("integ_1");
    std::shared_ptr<ast_operand> integ_op = std::make_shared<ast_operand>(var);
    expr_1->set_lhs(integ_op);

    expr_2 = std::make_shared<ast_expression>(ast_expression::ASSIGN);
    ...
    var = std::make_shared<variable>("intermediate_expression_0");
    std::shared_ptr<ast_definition> ie0_def = std::make_shared<ast_definition>("intermediate_expression_0", c_type_float, var);
    std::shared_ptr<ast_operand> ie0_op = std::make_shared<ast_operand>(var);
    std::shared_ptr<ast_expression> expr = std::make_shared<ast_expression>(ast_expression::MULT);
    expr->set_rhs(fwd_op);
    expr->set_lhs(ts_op);
    ie0_def->set_scalar_initializer(expr);

    var = std::make_shared<variable>("intermediate_expression_1");
    std::shared_ptr<ast_definition> ie1_def = std::make_shared<ast_definition>("intermediate_expression_1", c_type_float, var);
    std::shared_ptr<ast_operand> ie1_op = std::make_shared<ast_operand>(var);
    expr = std::make_shared<ast_expression>(ast_expression::MULT);
    expr->set_rhs(w_op);
    expr->set_lhs(ie0_op);
    ...
    std::shared_ptr<ast_expression> expr_as = std::make_shared<ast_expression>(ast_expression::ASSIGN);
...
TEST(HlPassesTest, dead_load_elimination) {
    ...
    std::shared_ptr<ast_definition> def = std::make_shared<ast_definition>("Ts", c_type_float, ts_var);
    input_root->add_content(def);

    std::shared_ptr<variable> cvar = std::make_shared<variable>("constant", 158.6f);
    std::shared_ptr<ast_operand> constant_op = std::make_shared<ast_operand>(cvar);
    std::shared_ptr<ast_operand> dest_op = std::make_shared<ast_operand>(ts_var);
    std::shared_ptr<ast_expression> exp = std::make_shared<ast_expression>(ast_expression::ASSIGN);
...
TEST(HlPassesTest, nested_function_inlining) {
    ...
    var->set_variable_class({variable_input_type, false});
    std::shared_ptr<ast_operand> in_1_op = std::make_shared<ast_operand>(var);
    var = std::make_shared<variable>("memory_2");
    var->set_variable_class({variable_memory_type, false});
    std::shared_ptr<ast_operand> mem_2_op = std::make_shared<ast_operand>(var);



    std::shared_ptr<ast_expression> exp_add = std::make_shared<ast_expression>(ast_expression::ADD);
    exp_add->set_rhs(mem_2_op);
    exp_add->set_lhs(in_1_op);


    var = std::make_shared<variable>("_fcmglr_function_level_2_out");
    std::shared_ptr<ast_definition> def_0 = std::make_shared<ast_definition>("_fcmglr_function_level_2_out", c_type_float, var);
    ...
    var = std::make_shared<variable>("inlined_variable_0");
    std::shared_ptr<ast_definition> def_1 = std::make_shared<ast_definition>("inlined_variable_0", c_type_int, var);

    std::shared_ptr<variable> var_const = std::make_shared<variable>("_fcmglr_function_level_2_constant", 120);
    std::shared_ptr<ast_operand> const_op = std::make_shared<ast_operand>(var_const);
    ...
    std::shared_ptr<ast_expression> expr_sat = std::make_shared<ast_expression>(ast_expression::SATP);
    ...
    var = std::make_shared<variable>("memory_2");
    var->set_variable_class({variable_memory_type, false});
    std::shared_ptr<ast_operand> op_dest = std::make_shared<ast_operand>(var);


    std::shared_ptr<ast_expression> expr = std::make_shared<ast_expression>(ast_expression::ASSIGN);
...
TEST(HlPassesTest, complex_division_implementation) {
    ...
    std::shared_ptr<ast_function_def> main = std::make_shared<ast_function_def>();
    main->set_name("main");
    main->set_return_type(c_type_int);
    gold_standard->add_content(main);

    std::vector<std::shared_ptr<ast_node>> main_body;

    std::shared_ptr<variable> a = std::make_shared<variable>("a");
    a->set_variable_class({variable_input_type, false});
    std::shared_ptr<ast_definition> a_def = std::make_shared<ast_definition>("a", c_type_float, a);
    main_body.push_back(a_def);

    std::shared_ptr<variable> b = std::make_shared<variable>("b");
    std::shared_ptr<ast_definition> b_def = std::make_shared<ast_definition>("b", c_type_float, b);
    std::shared_ptr<variable> b_const = std::make_shared<variable>("constant", 52.6f);
    std::shared_ptr<ast_operand> b_op = std::make_shared<ast_operand>(b_const);
    b_def->set_scalar_initializer(b_op);
    main_body.push_back(b_def);

    std::shared_ptr<variable> c = std::make_shared<variable>("c");
    std::shared_ptr<ast_definition> c_def = std::make_shared<ast_definition>("c", c_type_int, c);

    std::shared_ptr<ast_expression> mul_exp = std::make_shared<ast_expression>(ast_expression::MULT);
    mul_exp->set_lhs(std::make_shared<ast_operand>(a));


    mul_exp->set_lhs(std::make_shared<ast_operand>(a));

    std::shared_ptr<ast_expression> rec_exp = std::make_shared<ast_expression>(ast_expression::RECIPROCAL);
    rec_exp->set_rhs(std::make_shared<ast_operand>(b));
    mul_exp->set_rhs(rec_exp);

    std::vector<std::shared_ptr<ast_node>> args = {mul_exp};
    std::shared_ptr<ast_call> fti_call = std::make_shared<ast_call>("fti", args);
...
TEST(HlPassesTest, comparison_flipping) {
    ...
    auto init = std::static_pointer_cast<ast_definition>(result_ast->get_content()[1])->get_scalar_initializer();
    EXPECT_EQ(init->node_type,hl_ast_node_type_expr);
    auto exp = std::static_pointer_cast<ast_expression>(init);
    EXPECT_EQ(exp->get_type(), ast_expression::LTE);
