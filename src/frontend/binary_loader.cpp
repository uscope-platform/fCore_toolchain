...
namespace fcore{
    ...
    instruction_variant binary_loader::process_ternary_instr(uint32_t instruction) {
        ...
        bool operand_a_common = (instruction >> (fcore_opcode_width+3*fcore_register_address_width) & 0x1);
        bool operand_b_common = (instruction >> (fcore_opcode_width+3*fcore_register_address_width+1) & 0x1);
        bool operand_c_common = (instruction >> (fcore_opcode_width+3*fcore_register_address_width+2) & 0x1);


        std::string r_a_name = "r" + std::to_string(operand_a);
        std::string r_b_name = "r" + std::to_string(operand_b);
        std::string r_c_name = "r" + std::to_string(operand_c);


        std::shared_ptr<variable> op_a_var = std::make_shared<variable>(r_a_name);
        std::shared_ptr<variable> op_b_var  = std::make_shared<variable>(r_b_name);
        std::shared_ptr<variable> op_c_var  = std::make_shared<variable>(r_c_name);
        std::shared_ptr<variable> dest_var  = std::make_shared<variable>(r_a_name);
        ...
        if(operand_c_common) {
            op_c_var->set_name(r_c_name + "c");
            op_c_var->set_variable_class({variable_regular_type, true});
        }
