...
namespace fcore{
    ...
    std::shared_ptr<hl_ast_node> constant_folding_pass::process_node_by_type(std::shared_ptr<hl_ast_node> item) {
        if(!item->is_terminal()){
    ...
    std::shared_ptr<hl_ast_node> constant_folding_pass::process_terminal(std::shared_ptr<hl_ast_node> element) {
    ...
    std::shared_ptr<hl_ast_node> constant_folding_pass::process_expression(std::shared_ptr<hl_expression_node> exp) {
        if(expression_evaluator::is_constant_expression(exp)){
            return expression_evaluator::evaluate_expression(exp);
        } else return exp;
    }

    std::shared_ptr<hl_ast_node> constant_folding_pass::process_definition(std::shared_ptr<hl_definition_node> exp) {
    ...
    std::shared_ptr<hl_ast_node> constant_folding_pass::process_f_call(std::shared_ptr<hl_function_call_node> f_call) {

        std::vector<std::shared_ptr<hl_ast_node>> new_args;

        for(auto &item: f_call->get_arguments()){
            new_args.push_back(process_node_by_type(item));
        }
        f_call->set_arguments(new_args);
        return f_call;
    }
