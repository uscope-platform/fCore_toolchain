...
namespace fcore{
    ...
    std::shared_ptr<hl_code_block> dead_load_elimination::process_global(std::shared_ptr<hl_code_block> element, const std::vector<std::shared_ptr<hl_definition_node>> &globals) {
    ...
    void dead_load_elimination::search_usages(std::shared_ptr<hl_expression_node> element) {
        ...
        if(element->get_rhs()->node_type == hl_ast_node_type_operand){
            ...
            if(rhs_expr->get_type() == hl_expression_node::EFI){
                efi_mode = true;
    ...
    std::shared_ptr<hl_ast_node> dead_load_elimination::purge_dead_loads(std::shared_ptr<hl_ast_node> element) {
        if(element->node_type == hl_ast_node_type_definition) {
            ...
            if(node->get_type() == hl_expression_node::ASSIGN){
                ...
                std::string name = std::static_pointer_cast<hl_ast_operand>(node->get_lhs().value())->get_identifier();
