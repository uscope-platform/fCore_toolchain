...
namespace fcore{
    ...
    ast_operand::ast_operand(std::shared_ptr<variable> iv) : ast_node(hl_ast_node_type_operand) {
        inner_variable = std::move(iv);
    }


    std::string ast_operand::pretty_print() {
        std::string ret_val;
    ...
    std::shared_ptr<ast_operand> ast_operand::deep_copy(const std::shared_ptr<ast_operand> &orig) {
        std::shared_ptr<variable> new_var = variable::deep_copy(orig->get_variable());
        std::shared_ptr<ast_operand> copied_obj = std::make_shared<ast_operand>(new_var);

        std::vector<std::shared_ptr<ast_node>> new_array_idx;
        for(auto &item:orig->get_array_index()){
            new_array_idx.push_back(ast_node::deep_copy(item));
        }
