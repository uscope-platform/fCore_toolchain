...
uint32_t fcore::ll_conversion_instr_node::emit() {
...
void fcore::ll_conversion_instr_node::print() {
    std::cout << std::setfill('0') << std::setw(4) <<  std::hex << emit() << " -> OPCODE: " << opcode <<
              " SOURCE: " << source->to_str() << " DESTINATION: " << destination->to_str()<<std::endl;
}

int fcore::ll_conversion_instr_node::instruction_count() {
    return 1;
}

void fcore::ll_conversion_instr_node::set_arguments(const std::vector<std::shared_ptr<variable>> &a) {
    source = a[0];
    destination = a[1];
}

std::string fcore::ll_conversion_instr_node::disassemble() {
    return opcode + " " + source->get_name() + ", " + destination->get_name();
}

nlohmann::json fcore::ll_conversion_instr_node::dump() {
